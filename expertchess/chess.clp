(defglobal ?*N* = 8)			; *N* x *N* board, =8 for now
...
(deffunction read-pos (?str)
  ...
  (switch ?str
  	(case 1 then (open 1.txt data1 "r") (bind ?num (letter-to-number (read data1))))
  	(case 2 then (open 2.txt data2 "r") (bind ?num (letter-to-number (read data2))))
  	(case 3 then (open 3.txt data3 "r") (bind ?num (letter-to-number (read data3))))
  	(case 4 then (open 4.txt data4 "r") (bind ?num (letter-to-number (read data4))))
  	(case 5 then (open 5.txt data5 "r") (bind ?num (letter-to-number (read data5))))
  	(case 6 then (open 6.txt data6 "r") (bind ?num (letter-to-number (read data6))))
  )
...
attack) - reenter black king coordinates"
  ...
  (if (not (and (bind ?wkv (read-pos 1))
		(bind ?wkh (read-pos
			    2))
		(bind ?wrv (read-pos 3))
		(bind ?wrh (read-pos
  ...
  (if
      (and (= ?wkv ?wrv) (= ?wkh ?wrh))
     then
    (open "output.txt" ill "w")
    (printout ill "illegal")
    (halt)
    ...
    (if (not (and (bind ?wrv (read-pos
		  ...
		  (bind ?wrh (read-pos
			      4))))
	then (return FALSE)))
  (if (not (and (bind ?bkv (read-pos 5))
		(bind ?bkh (read-pos
			    6))))
      then (return FALSE))
...
(deffunction print-move (?p ?v ?h ?delim)
  ...
  (printout out ?delim " " (piece-to-letter ?p)
...
(deffunction enter-black-move (?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
  ...
  (while (not (find-move-in-sequence ?v ?h ?moves)) do
  	(open "output.txt" ill1 "w")
	 (printout ill1 "illegal")
	 (halt)
...
(defrule too-many-moves
  ...
  =>
  (open "output.txt" out1 "w")
  (printout out1 "tie" crlf)
  (halt)
  (reset))
...
(defrule check-for-mate
  ...
  (if (check-for-mate ?wkv ?wkh ?wrv ?wrh ?bkv ?bkh)
      then 
      (open "output.txt" out2 "w")
      (printout out2 "winner" crlf)
      (halt)
      (reset)
